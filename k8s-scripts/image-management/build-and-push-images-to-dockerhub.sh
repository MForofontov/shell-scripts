...
usage() {
  ...
  echo "  This script builds Docker images and pushes them to Docker Hub or any Docker registry."
  echo
  echo -e "\033[1;34mUsage:\033[0m"
  echo "  build-and-push-images-to-dockerhub.sh -u <docker-username> -p <docker-pat> -f <images.txt> -m <manifest-dir> [--log <log_file>]"
  echo
...
parse_args() {
  ...
  if [[ -z "$DOCKER_USERNAME" || -z "$DOCKER_PAT" ]]; then
    log_message "ERROR" "Docker username and PAT are required."
    usage
  fi
}
