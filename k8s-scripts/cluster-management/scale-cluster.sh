...
usage() {
  ...
  echo -e "  \033[1;33m-n, --name\033[0m NAME          Cluster name to scale (required)"
  echo -e "  \033[1;33m-p, --provider\033[0m PROVIDER  Provider to use (minikube, kind, k3d) (default: ${PROVIDER})"
  echo -e "  \033[1;33m-c, --nodes\033[0m COUNT        Target number of nodes (required, must be ≥ 1)"
  echo -e "  \033[1;33m-f, --force\033[0m              Force scaling without confirmation"
  echo -e "  \033[1;33m-t, --timeout\033[0m SECONDS    Timeout in seconds for operation (default: ${WAIT_TIMEOUT})"
