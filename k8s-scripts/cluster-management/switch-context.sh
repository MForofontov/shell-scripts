...
usage() {
  print_with_separator "Kubernetes Context Switching Tool"
  ...
  echo -e "  \033[1;33m-c, --context\033[0m CONTEXT    Switch to specified context"
  echo -e "  \033[1;33m-p, --provider\033[0m PROVIDER  Filter contexts by provider (minikube, kind, k3d)"
  echo -e "  \033[1;33m-i, --interactive\033[0m        Select context interactively"
  echo -e "  \033[1;33m-a, --all\033[0m                Show all contexts (including non-cluster contexts)"
  echo -e "  \033[1;33m--filter\033[0m PATTERN         Filter contexts by name pattern"
...
# Main function
main() {
  print_with_separator "Kubernetes Context Switcher"
