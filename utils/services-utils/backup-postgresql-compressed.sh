...
log_message "INFO" "Starting PostgreSQL backup process for database: $DB_NAME"
print_with_separator "PostgreSQL Backup"
...
log_message "INFO" "Creating compressed backup at $BACKUP_FILE..."
if pg_dump -U "$DB_USER" "$DB_NAME" | gzip > "$BACKUP_FILE"; then
  log_message "SUCCESS" "Database backup created and compressed at $BACKUP_FILE."
else
  print_with_separator "End of PostgreSQL Backup"
