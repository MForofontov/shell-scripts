...
log_message "INFO" "Starting scan for unused SSH keys in $SSH_DIR..."
print_with_separator "Unused SSH Key Detector"
...
for user in $(ls "$SSH_DIR"); do
  ...
  if [ -d "$USER_SSH_DIR" ]; then
    ...
    find "$USER_SSH_DIR" -type f -name "*.pub" -exec ls -l {} \; | tee -a "$LOG_FILE"
  else
