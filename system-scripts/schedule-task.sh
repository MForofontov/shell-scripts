...
main() {
  ...
  format-echo "INFO" "Checking if the script is already scheduled..."
  if crontab -l 2>/dev/null | grep -q -F "$SCRIPT_PATH"; then
    format-echo "INFO" "The script is already scheduled."
  else
    # Schedule the script using cron
    (crontab -l 2>/dev/null; echo "$CRON_SCHEDULE $SCRIPT_PATH") | crontab -
    format-echo "SUCCESS" "The script has been scheduled to run at '$CRON_SCHEDULE'."
  fi

  print_with_separator "End of Schedule Task Script"
  format-echo "SUCCESS" "Task scheduling completed."
}
